...
> 🔐 In our case, the CodeJam facilitator will provide you with the URL and credentials required to call the integration flow. That said, if you are really adventurous ⛵️ or really familiar with SAP Cloud Integration, you can import the integration package included in the assets folder of this exercise (`./assets/cloud-integration/ManageAPIsCodeJam-IntegrationPackage.zip`) and deploy the integration flow in your Cloud Integration instance. Once you've done that, you will also need to create a service key (Process Integration Runtime - Integration flow type) to be able to call the integration flow.
